...
"""
    pythonwithgittest.controllers.messaging_controller
...
class MessagingController(BaseController):
    ...
    def get_sms_status(self,
        ...
        if _context.response.status_code == 400:
            raise APIException('Invalid or missing request parameters  NOT-PROVISIONED   Request flagged as containing suspicious content', _context)
            ...
            raise APIException('The requested URI does not exist   OLD-NONEXISTANT-MESSAGE-ID   RESOURCE-NOT-FOUND', _context)
    ...
    def create_send_sms(self,
        ...
        Returns:
            MessageSentResponse: Response from the API. Created
        ...
        # Endpoint and global error handling using HTTP status codes.
        if _context.response.status_code == 400:
            raise APIException('Invalid or missing request parameters   TO-MSISDN-NOT-VALID   SENDER-MISSING   DELIVERY-IMPOSSIBLE   FROM-MSISDN-TOO-LONG   BODY-TOO-LONG   BODY-MISSING   TO-MSISDN-TOO-LONG   TECH-ERR   BODY-NOT-VALID   NOT-PROVISIONED   Request flagged as containing suspicious content', _context)
        elif _context.response.status_code == 401:
            raise APIException('Invalid access token. Please try with a valid token', _context)
        elif _context.response.status_code == 403:
            raise APIException('Authorization credentials passed and accepted but account does not have permission   SpikeArrest-The API call rate limit has been exceeded', _context)
    ...
    def create_send_mms(self,
        ...
        # Endpoint and global error handling using HTTP status codes.
        if _context.response.status_code == 400:
            raise APIException('Invalid or missing request parameters      MMS-TYPE-MISSING   MMS-PAYLOAD-MISSING    MMS-FILENAME-MISSING   DELIVERY-IMPOSSIBLE    TO-MSISDN-NOT-VALID    SENDER-MISSING   DELIVERY-IMPOSSIBLE   SUBJECT-TOO-LONG   FROM-MSISDN-TOO-LONG   TO-MSISDN-TOO-LONG   NOT-PROVISIONED   Request flagged as containing suspicious content', _context)
        elif _context.response.status_code == 401:
            raise APIException('Invalid access token. Please try with a valid token', _context)
        elif _context.response.status_code == 403:
            raise APIException('Authorization credentials passed and accepted but account does not have permission   SpikeArrest-The API call rate limit has been exceeded', _context)
